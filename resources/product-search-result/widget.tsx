...
const ProductSearchResult: React.FC = () => {
  const { isPending, props, displayMode, requestDisplayMode, state, setState } =
  ...
  const dashboardDoc = React.useMemo(() => {
    const injected = `<script>window.__UC_INITIAL_PLAYHEAD_MS=${initialPlayheadMs};</script>`;
  ...
  React.useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      const data = event.data as { type?: string; ms?: number } | undefined;
      if (!data || data.type !== "uc-dashboard-playhead") return;
      const ms = data.ms;
